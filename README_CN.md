...
- `RegisterScrapeHook(ctx context.Context)` 此前为 TODO 存根，现已实现：调用 `countOrphanLeases` 并更新该指标。通过 `main.go` 中的 `metrics.RegisterScrapeHook(svc.RegisterScrapeHook)` 接入（chassis v0.1.3 API）。
