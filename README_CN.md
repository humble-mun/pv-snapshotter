...
- **DaemonSet**，包含两个容器：
  - `config`（原生 sidecar，`restartPolicy: Always`）：等待 daemon 的 `/readyz` 端点就绪，幂等地修补 `/etc/containerd/config.toml`（复制基础 runtime 配置并添加 `snapshotter = "pv-snapshotter"`），必要时通过 cgo nsenter 前置逻辑重启 containerd，然后阻塞至 Pod 终止。
  - `daemon`：pv-snapshotter gRPC 代理快照器。
- 每个 `containerdConfig.runtimeClasses` 条目对应一个 **RuntimeClass**，命名为 `<基础名><后缀>`。
- **ServiceAccount**，设置 `automountServiceAccountToken: false`（无需访问 Kubernetes API）。
