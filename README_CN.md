...
2. **查找关联 PVC**：查找 PVC 后等待最多 `boundTimeout`（默认 10 s）使其进入 Bound 状态。若超时未绑定则拒绝 Pod——pv-snapshotter 无法在未绑定的卷上准备 overlay upperdir，提前放行只是将失败延迟到节点层。PVC 名称按以下顺序解析：
   - **Pod 级覆盖**：若 Pod 设置了 `<annotation-prefix>/pvc-name-template` 注解（例如 `pv-snapshotter.humble-mun.io/pvc-name-template`；前缀即可配置的 `--annotation-prefix`，后缀固定），其值作为 Go 模板渲染（变量与 `pvcNameTemplate` 相同），结果即为 PVC 名称。这样 Pod 可以绑定生命周期与 Pod／owner 名称无关的 PVC（例如启动时从多个预置 PVC 中选择其一）。该值也可以是不含任何模板占位符的固定 PVC 名称。渲染结果为空时直接拒绝，而不会回退。
   - **全局名称模板**：否则渲染 `pvcNameTemplate`（默认 `{{.OwnerName}}`），按渲染结果查找 PVC。
   - **全局选择器模板**：当 `pvcNameTemplate` 渲染为空时，使用 `pvcSelectorTemplate` 列出 PVC 并取第一个匹配项。
...
| 2 | Webhook | `.PVName`、`.VolumeHandle`、`.OwnerName`、`.PodName`、`.PodIndex` | 解析存储侧变量；将注解打到 Pod 上 |
| 3 | pv-snapshotter | `.PodUID`、`.PodName`、`.PodNamespace`、`var.*` | 在 `Mounts()` 时解析 Pod 身份变量 |

> **关于 `.PodIndex` 的说明：** 该值直接取自 Pod 的 `apps.kubernetes.io/pod-index` label（StatefulSet 与 Indexed Job 的 Pod，Kubernetes 1.28+，KEP-2164）。不带该 label 的 Pod——例如普通 Deployment/DaemonSet，或非索引型工作负载——渲染出的 `.PodIndex` 会是空字符串。模板编写者需要自行处理这种情况（例如通过条件判断，或仅在面向索引型工作负载的模板中使用 `.PodIndex`），不能假定该值一定是数字。
...
| `webhook.pvcNameTemplate` | `{{.OwnerName}}` | Go 模板 → 要查找的 PVC 名称（Pod 可通过 `<annotationPrefix>/pvc-name-template` 注解按 Pod 覆盖）|
...
### v0.1.9 — 新增 `.PodIndex` 模板变量（当前版本）

- **`.PodIndex` 模板变量。** Webhook 的 `templateData` 新增 `PodIndex` 字段，取自 Pod 的 `apps.kubernetes.io/pod-index` label（StatefulSet 与 Indexed Job 的 Pod，Kubernetes 1.28+，KEP-2164），可在 `--webhook-pvc-name-template`、`--webhook-pvc-selector-template`、`--webhook-annotation-templates` 以及 Pod 级 `<annotation-prefix>/pvc-name-template` 覆盖模板中使用。该值为 label 的直接透传，不做校验：不带该 label 的 Pod 会渲染为空字符串而不是报错，因此依赖该变量的模板需要自行处理这种情况。

### v0.1.8 — 共享 annotation 包 + Pod 级 PVC 覆盖

- **共享 `pkg/annotation` 包。** 将 `--annotation-prefix` 相关逻辑（flag 注册、RFC 1123 校验、前缀解析、注解键拼接）抽取到一个无构建约束的共享包，供快照器 resolver 与变更 Webhook 复用。该 flag 现由 `annotation.RegisterFlags` 注册（在 `main.go` 中接入）；校验函数与保留域名常量改为包内私有。
- **Pod 级 PVC 名称覆盖。** Pod 可设置 `<annotation-prefix>/pvc-name-template` 注解（后缀固定，前缀通过 `--annotation-prefix` 可配置）来覆盖全局的 `--webhook-pvc-name-template` / `--webhook-pvc-selector-template`。其值为 Go 模板（变量与名称模板相同）或固定 PVC 名称；渲染结果为空时直接拒绝。这样 Pod 可以绑定生命周期与 Pod／owner 名称无关的 PVC。
- **Helm：消除 map 覆盖泄漏。** `webhook.annotationTemplates` 与 `webhook.objectSelector` 在 `values.yaml` 中默认改为 `{}`。Helm 对 map 值按键深合并、且永不丢弃 chart 默认键，因此非空 map 默认值会泄漏进用户 `-f` 覆盖中。真实默认值现放到合并触及不到的地方：`annotationTemplates` 回退到二进制内置默认值（仅当该 map 非空时 DaemonSet 才传 `--webhook-annotation-templates`），`objectSelector` 的 opt-in 标签（`pv-snapshotter.humble-mun.io/inject: "true"`）移入 `webhook.yaml` 模板的 `else` 分支。用户提供的值现在会完整**替换**默认值，而非按键合并。
- **chassis 升级至 v0.1.10（配置加载修复）。** 将 `github.com/humble-mun/chassis` 从 v0.1.7 升级到 v0.1.10：把全局 viper 的 `SetConfigName`/`AddConfigPath` 延迟到配置加载闭包内执行。此前注册 `config` 子命令会在共享的全局 viper 上覆盖根命令的配置名，导致守护进程读取了错误的文件并静默回退到编译期默认值（`root-path` 为空、绑定 `0.0.0.0:8080`、默认 TLS 路径 → 崩溃）。现守护进程可稳定加载 `/etc/humble-mun/daemon.yaml`。

### v0.1.7 — 孤立 lease GC、抓取钩子、字符串常量重构
