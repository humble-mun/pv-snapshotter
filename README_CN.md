...
> 并进入 NotReady 状态。请升级到修复了两个根本原因的 **v0.1.4**。
...
### v0.1.4 — 磁盘压力修复 + runtime_platforms 自动镜像解压

#### Bug 1 — `Cleanup()` 未转发 → 孤立快照目录永久积累
...
嵌入的是 `snapshots.Snapshotter` 接口而非具体类型，类型断言始终失败并返回
`ErrNotImplemented` — 导致每个被删除的容器都会在磁盘上留下永久性的孤立目录。
...
#### Bug 2 — 缺少 `runtime_platforms` → 镜像层未解压进 pv-snapshotter

**根本原因。** containerd 根据 kubelet 在 CRI `PullImageRequest` 中传递的
`runtimeHandler` 字段（cri-api v0.29.0 / Kubernetes 1.29+）来选择镜像解压使用的
快照器。没有匹配的 `runtime_platforms` 条目时，containerd 回退到全局默认快照器
（`overlayfs`）解压镜像，但容器创建时使用 pv-snapshotter — 导致 `missing parent
snapshot` 错误。

**修复。** config sidecar（`patcher.go`）现在会在每个 runtime 条目旁同时注入
`runtime_platforms` 条目：
...
kubelet 为 `runtimeClassName: runc-pv` 的 Pod 拉取镜像时，containerd 会自动将镜像
解压到 pv-snapshotter 中。Kubernetes 1.29+ 无需手动执行 `ctr pull`。

#### 磁盘压力事故根因总结

v0.1.3 同时存在以上两个 Bug：

1. 镜像层被解压到默认 overlayfs 而非 pv-snapshotter → 容器创建时 `Prepare()` 报
   `missing parent snapshot`。
2. 删除的容器留下永久性孤立目录 → 磁盘无限增长。

**请勿在生产环境中运行 v0.1.3。** 请升级到 v0.1.4。
